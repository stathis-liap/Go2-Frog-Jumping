...
After completing all steps, your workspace should look like this:

```
~/Quadruped_Project/
├── go2_env/                          # Python virtual environment
├── unitree_sdk2py/                   # Unitree Python SDK
├── unitree_mujoco/                   # Unitree MuJoCo simulator
│   └── simulate_python/
│       ├── unitree_mujoco.py
│       ├── simulation_auto.py        # ← moved here from frog_jump/ (Step 6)
│       └── ...
└── frog_jump/                        # This repository
    ├── main_jump.py
    ├── config.py
    └── ...
```
...
# Install the MuJoCo engine and pygame
...
### 6. Move `simulation_auto.py` into the Simulator

The `frog_jump/` folder contains a file called `simulation_auto.py` that must be placed inside the MuJoCo simulate_python directory to work correctly.

```bash
mv ~/Quadruped_Project/frog_jump/simulation_auto.py \
   ~/Quadruped_Project/unitree_mujoco/simulate_python/
```
...
To run the experiment, open **two separate terminal windows**. Make sure your virtual environment is activated in both.
